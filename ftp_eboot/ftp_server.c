...
/* ---------- minimal formatter ---------- *
 * Cell SDK's ux_vsnprintf pulls more libc state than this tiny application
 * needs. Roll our own with just the
...
static void handle_session(int ctrl) {
    ...
    send_reply(&s, "220 Standalone PS3 FTP ready");
...
static int boot_setup(void) {
    ...
    /* libnet needs a 128 KB scratch buffer. Pull from kernel pages so the
     * executable's small process heap stays available for client work. */
    sys_addr_t mem = 0;
