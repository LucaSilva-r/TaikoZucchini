...
Cell SDK ツリーを用意し、`CELL_SDK` をそこに向けます。Windows では、
ネイティブの `host-win32/` ツールチェーンを使います：
...
### Windows と Visual Studio NMAKE

x64 Native Tools Command Prompt を開くか、`cmd.exe` から Visual Studio の
ビルド環境を初期化し、リポジトリのルートからビルドします：

```bat
nmake /f Makefile.win TAIKO_ZUCCHINI_API_TOKEN=your_token
```

`Makefile.win` は `%SCE_PS3_ROOT%` が設定されていればそれを使い、なければ
`H:\PS3_SDK` を使います。必要なら明示的に上書きできます：
...
従来の `Makefile` も GNU Make 用に残しています。`host-win32/` があればそれを、
なければ `host-linux/` を自動選択するため、Linux、MSYS、Git Bash、Wine ラッパー
SDK から使えます：
...
- `Makefile`：GNU Make 用の Cell SDK SPRX ビルド。
- `Makefile.win`：SDK の `host-win32/` ツールを使う Visual Studio NMAKE ビルド。
