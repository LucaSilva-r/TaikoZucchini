/*
 * Big-endian byte array bignum.
 * Originally Copyright 2007-2010 Segher Boessenkool, GPLv2.
 * Ported from scetool bn.cpp by Luca Silva.
 */

#include <string.h>

#include "sce_bn.h"

static void bn_zero(uint8_t *d, uint32_t n) { memset(d, 0, n); }

void bn_copy(uint8_t *d, uint8_t *a, uint32_t n) { memcpy(d, a, n); }

int bn_compare(uint8_t *a, uint8_t *b, uint32_t n) {
    for (uint32_t i = 0; i < n; i++) {
        if (a[i] < b[i]) return -1;
        if (a[i] > b[i]) return  1;
    }
    return 0;
}

static uint8_t bn_add_1(uint8_t *d, uint8_t *a, uint8_t *b, uint32_t n) {
    uint32_t dig;
    uint8_t  c = 0;
    for (uint32_t i = n - 1; i < n; i--) {
        dig  = a[i] + b[i] + c;
        c    = (uint8_t)(dig >> 8);
        d[i] = (uint8_t)dig;
    }
    return c;
}

static uint8_t bn_sub_1(uint8_t *d, uint8_t *a, uint8_t *b, uint32_t n) {
    uint32_t dig;
    uint8_t  c = 1;
    for (uint32_t i = n - 1; i < n; i--) {
        dig  = a[i] + 255u - b[i] + c;
        c    = (uint8_t)(dig >> 8);
        d[i] = (uint8_t)dig;
    }
    return (uint8_t)(1 - c);
}

void bn_reduce(uint8_t *d, uint8_t *N, uint32_t n) {
    if (bn_compare(d, N, n) >= 0)
        bn_sub_1(d, d, N, n);
}

void bn_add(uint8_t *d, uint8_t *a, uint8_t *b, uint8_t *N, uint32_t n) {
    if (bn_add_1(d, a, b, n))
        bn_sub_1(d, d, N, n);
    bn_reduce(d, N, n);
}

void bn_sub(uint8_t *d, uint8_t *a, uint8_t *b, uint8_t *N, uint32_t n) {
    if (bn_sub_1(d, a, b, n))
        bn_add_1(d, d, N, n);
}

static const uint8_t inv256[0x80] = {
    ,
};

static void bn_mon_muladd_dig(uint8_t *d, uint8_t *a, uint8_t b,
                              uint8_t *N, uint32_t n) {
    uint32_t dig;
    uint8_t z = (uint8_t)(-(d[n-1] + a[n-1]*b) * inv256[N[n-1]/2]);

    dig = d[n-1] + a[n-1]*b + N[n-1]*z;
    dig >>= 8;

    for (uint32_t i = n - 2; i < n; i--) {
        dig += d[i] + a[i]*b + N[i]*z;
        d[i+1] = (uint8_t)dig;
        dig >>= 8;
    }

    d[0] = (uint8_t)dig;
    dig >>= 8;

    if (dig)
        bn_sub_1(d, d, N, n);

    bn_reduce(d, N, n);
}

void bn_mon_mul(uint8_t *d, uint8_t *a, uint8_t *b, uint8_t *N, uint32_t n) {
    uint8_t t[64];
    bn_zero(t, n);
    for (uint32_t i = n - 1; i < n; i--)
        bn_mon_muladd_dig(t, a, b[i], N, n);
    bn_copy(d, t, n);
}

void bn_to_mon(uint8_t *d, uint8_t *N, uint32_t n) {
    for (uint32_t i = 0; i < 8u*n; i++)
        bn_add(d, d, d, N, n);
}

void bn_from_mon(uint8_t *d, uint8_t *N, uint32_t n) {
    uint8_t t[64];
    bn_zero(t, n);
    t[n-1] = 1;
    bn_mon_mul(d, d, t, N, n);
}

static void bn_mon_exp(uint8_t *d, uint8_t *a, uint8_t *N, uint32_t n,
                       uint8_t *e, uint32_t en) {
    uint8_t t[64];
    bn_zero(d, n);
    d[n-1] = 1;
    bn_to_mon(d, N, n);

    for (uint32_t i = 0; i < en; i++)
        for (uint8_t mask = 0x80; mask != 0; mask >>= 1) {
            bn_mon_mul(t, d, d, N, n);
            if ((e[i] & mask) != 0)
                bn_mon_mul(d, t, a, N, n);
            else
                bn_copy(d, t, n);
        }
}

void bn_mon_inv(uint8_t *d, uint8_t *a, uint8_t *N, uint32_t n) {
    uint8_t t[64], s[64];
    bn_zero(s, n);
    s[n-1] = 2;
    bn_sub_1(t, N, s, n);
    bn_mon_exp(d, a, N, n, t, n);
}
